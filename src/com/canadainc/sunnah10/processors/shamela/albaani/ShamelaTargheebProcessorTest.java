...
public class ShamelaTargheebProcessorTest
{
	...
	public void testContinuedNarration() throws IOException
	{
		...
		ShamelaTestUtils.loadAndAssertSize("targheeb/0101.txt", s, 1);
		ShamelaTestUtils.loadAndAssertSize("targheeb/0102.txt", s, 2);
		ShamelaTestUtils.assertNarration(s.getNarrations().get(0), 1, "صحيح",
		...
		ShamelaTestUtils.assertNarration(s.getNarrations().get(1), 2, "صحيح",
	...
	{
		ShamelaTestUtils.loadAndAssertSize("targheeb/0106.txt", s, 2);
		ShamelaTestUtils.assertNarration(s.getNarrations().get(0), 9, "حسن لغيره",
				"رواه الطبراني بإسناد لا بأس");
		ShamelaTestUtils.assertNarration(s.getNarrations().get(1), 10, "صحيح",
	...
	public void testMultiChapter() throws IOException
	{
		...
		ShamelaTestUtils.loadAndAssertSize("targheeb/0150.txt", s, 3);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(0), 97, "صحيح",
				"أحدِهما، قدّمه في اللحدِ. رواه البخاري");
		assertEquals(c, s.getNarrations().get(0).chapter);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(1), 98, "حسن",
				"عنه، وإكرامَ ذي السلطان المُقْسِطِ");
		assertEquals(c, s.getNarrations().get(1).chapter);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(2), 99, "صحيح",
				"صحيح على شرط مسلم");
	...
	public void testBookNoBrackets() throws IOException
	{
		...
		ShamelaTestUtils.loadAndAssertSize("targheeb/0135.txt", s, 1);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(0), 67, "صحيح",
	...
	public void numberedList() throws IOException
	{
		...
		ShamelaTestUtils.loadAndAssertSize("targheeb/0357.txt", s, 2);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(1), 552, "صحيح",
	...
	{
		ShamelaTestUtils.loadAndAssertSize("targheeb/1709.txt", s, 1);

		ShamelaTestUtils.assertNarration(s.getNarrations().get(0), 3492, "صحيح",
	...
	{
		ShamelaTestUtils.loadAndAssertSize("targheeb/1057.txt", s, 3);

		Narration n = s.getNarrations().get(0);
		ShamelaTestUtils.assertNarration(n, 1964, "صحيح",
		...
		ShamelaTestUtils.assertNarration(n, 1965, "حسن لغيره",
				"صحيح الإسناد");
		assertNull( n.chapter);
		assertNull( n.book);
		...
		ShamelaTestUtils.assertNarration(n, 1966, "حسن صحيح",
				"رواه ابن حبان في");
		assertNull( n.chapter);
		assertNull( n.book);
